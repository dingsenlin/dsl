# Generated by Django 2.2.2 on 2019-06-13 13:17

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('comments', '0002_auto_20190613_1930'),
    ]

    operations = [
        migrations.RemoveField(
            model_name='comment',
            name='contset',
        ),
        migrations.RemoveField(
            model_name='comment',
            name='enail',
        ),
        migrations.AddField(
            model_name='comment',
            name='content',
            field=models.TextField(default=None, max_length=500, verbose_name='正文'),
        ),
        migrations.AddField(
            model_name='comment',
            name='email',
            field=models.EmailField(blank=True, max_length=254, null=True, verbose_name='邮箱'),
        ),
        migrations.AlterField(
            model_name='comment',
            name='name',
            field=models.CharField(max_length=30, verbose_name='昵称'),
        ),
        migrations.AlterField(
            model_name='comment',
            name='url',
            field=models.URLField(blank=True, null=True, verbose_name='个人中心'),
        ),
    ]
